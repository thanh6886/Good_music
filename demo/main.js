...
function setTimeSonds(_Time){
      var h = 0, m = 0, s;
      _Time = parseInt(_Time, 10);
      if (_Time > 60 * 60) {
       h = parseInt(_Time / (60 * 60), 10);
       _Time -= h * 60 * 60;
      }
      if (_Time > 60) {
       m = parseInt(_Time / 60, 10);
       _Time -= m * 60;
      }
      s = _Time;
      _Time = (h > 0)? h + ':' : '';
      _Time += (m > 0)? ((m < 10 && h > 0)? '0' : '') + m + ':' : '0:';
      _Time += ((s < 10)? '0' : '') + s;
        return _Time
}


function render(data){                         // lấy danh sách bài hát từ api dùng map để load ra mảng 
...
function loadSong(data, count){                                      // load ra bài hát muốn chạy data chuyền vào là 1 mảng 
      const currentsong = data[count]
           audio.src = currentsong.path
            heading.innerText = currentsong.name
            cdThumb.style.backgroundImage = `url(${currentsong.image})`
}
playBtn.onclick = function play_pause_Track(){                    //  sử lý sự kiện khi bấm play và khi bấm pause
          isPlaying ? pauseSound() : playSound();
            cdRun()
}
function playSound(){                                                              // khi bài hát play
      audio.play();
      isPlaying = true
      nowPlaying.innerText = "PLAYING >_<"
      playBtn.innerHTML = '<i class="fa fa-pause-circle fa-3x"></i>'
}
function pauseSound(){                                                              // khi bài hat pause
      audio.pause();
      isPlaying = false
...
repeatBtn.onclick = function(e){                                                   // sự kiện khi bấm nút repeat
          ...
          if(isRepeatBtn == false){
            ...
            repeatBtn.innerHTML = '<i class="fas fa-redo fa-2x" style="color: brown;"></i>'
          }
}

randomBtn.onclick = function(e){                                                    // sự kiện khi bấm  nút random
      isRandom =! isRandom
      if(isRandom == false){
        ...
        randomBtn.innerHTML = '<i class="fas fa-random fa-2x" style="color: brown;"></i>'
      }
}
function getRandom(data){                                                                // random song 
            let randomIndex  = Math.floor(Math.random() * lengthData)
            return randomIndex
}

_volume.addEventListener("input", (e)=>{                                                   // sự kiện chỉnh thanh âm thanh
            var Volume = (e.target.value)
            currentVolume.innerText = parseInt(Volume*100)
            audio.volume = Volume
})

progress.onchange = function(){                                                    // bắt sự kiện sử lý khi tua thanh input 
      const seekto = audio.duration*(progress.value / 100);                             
      audio.currentTime = seekto
}
audio.ontimeupdate = function(){                                                          // bắt sự kiện khi bài hát chạy thì input sẽ  chạy
      if(audio.duration){
...
document.onscroll = function(){                                                   // kéo danh sách lên 
...
function appRun(){                  // func để chạy tt
      ...
      getCourses((data)=>{
                  ...
                  nextBtn.onclick = function nextClick(){   // chuyển bài tiếp  
                  ...
                  prevBtn.onclick = function(){         // quay lại bài cũ
                        if(count > 0){
                              count--
                        }
                        else{
                  ...
                  playlist.onclick = function(e){                                       // khi bấm vào danh sách songs
                 ...
                 audio.addEventListener("ended", function(){                                    // bắt sự kiện bài hát khi hết 
                              if(isRepeatBtn === true && isRandom === false){                        // khi nút repeat bật random tắt
                                    audio.play()
                               }
                               else if(isRepeatBtn === false && isRandom === true){         //  bật random tắt repeat
                               }
                                  // Lỗi khi bật  random sẽ next sang bài khác rồi tắt random không kiểm soát dc biến count 

                              //     count = getRandom(data)    
                              //     loadSong(data, count)
                              //     playSound()
                              //     cdRun()
                              else if(isRepeatBtn === false && isRandom === false){                                                 // bật hoặc tắt cả 2 lỗi  :)
                                    nextBtn.click()
                              }  
